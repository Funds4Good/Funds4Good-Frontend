...
const MyApplications = () => {
    ...
    useEffect(() => {
        ...
        BaseUrl.get("/api/loans/getUserLoans", {
            headers: {
            ...
            {applications.map((application) => (
                <div key={application.loanRequestId} className="bg-white border border-gray-200 rounded-lg w-full shadow-md p-4 mb-4 flex flex-col justify-between">
                    <div className="flex items-center mb-4">
                        <Image
                            src={"image-source-here"} // Provide valid source for the profile image
                            alt="Profile"
                    ...
                    <div className="flex items-start gap-6 mb-6">
                        ...
                        <div>Duration: <span className="text-[#808080]">{application.timeDuration}</span></div>
                    </div>
                    <div className="mb-6"> <ProgressBar progress={application.raising / application.loanAmount * 100} /></div>
                    <div className="flex items-start justify-between gap-6 mb-6">
                        <div>Raising: <span className="text-[#51DA21]">{application.raising}</span></div>
                        <div>Amount Left: <span className="text-[#51DA21]">{application.loanAmount - application.raising}</span></div>
                    </div>
